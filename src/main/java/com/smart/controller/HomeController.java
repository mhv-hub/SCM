...
public class HomeController {
	...
	// Below annotation is used to get object of this at the runtime. ( IOC container will create the object and will inject it here )
	@Autowired
	...
	// Data can be sent to View using HttpSession object as well, but that can only have key value pairs doe String.
	// To send any POJO object ro View, Model object is needed.
	// using model.addAttribute() method, we can send an object with a key name to View.
	// on the View ( HTML code ) , we can use thymleaf to get the objects sent by controller using model object.
	@RequestMapping("/")
	public String home(Model model) {
		model.addAttribute("title", "Home - Smart Contact Manager");
		// Below return string is the name of html page that will be loaded when this url is fired.
		return "home";
	}
	...
	// @ModelAttribute("user") User user is telling that the user data sent by View needs to be validated.
	// BidingResult result : this result object will store the result of validation done by Spring.
	// Validation will be done based on the rules defined by validation annnotations inside Entity class.
	// @RequestParam is used to get value from html form element. value = 'aggrement' is the name of the html field whose value will be fetched.
	...
	public String registerUser(@Valid @ModelAttribute("user") User user, BindingResult result,
		...
		try {
			...
			// this method will return true if any or all of the validation rules are broken.
			if (result.hasErrors()) {
			...
			// Possible because we have defined the class as entity and also used @Table to define which table is related to this.
